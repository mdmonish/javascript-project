...
const displayTransactionMovement = function (user, sort = false) {
  ...
  movs.forEach(function (mov, i) {
    ...
    <div class="movements__value">${priceFormatter(
      user.locale,
      user.currency,
      mov
    )}</div>
...
const displayMovementBalance = function (user) {
  user.balance = user.movements.reduce(function (acc, curr) {
    return acc + curr;
  }, 0);

  labelBalance.textContent = `${priceFormatter(
    user.locale,
    user.currency,
    user.balance
  )}`;
...
const calcSummaryMovement = function (user) {
  ...
  const withdrawn = user.movements
    .filter(mov => mov < 0)
    .reduce((acc, dpt) => acc + dpt, 0);
  labelSumOut.textContent = priceFormatter(
    user.locale,
    user.currency,
    Math.abs(withdrawn.toFixed(2))
  );

  const intrest = user.movements
    .filter(mov => mov > 0)
    .map(amt => (amt * user.interestRate) / 100)
    .filter(val => val >= 1)
    .reduce((acc, int) => acc + int, 0);
  labelSumInterest.textContent = priceFormatter(
    user.locale,
    user.currency,
    Math.abs(intrest.toFixed(2))
  );
};
